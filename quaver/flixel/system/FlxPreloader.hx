...
class FlxPreloader extends FlxBasePreloader
{
	...
	override function create():Void
	{
		...
		_buffer.addChild(new Bitmap(new BitmapData(_width, _height, false, 0x035e00)));
		...
		_bmpBar = new Bitmap(new BitmapData(1, 7, false, 0x5fff5f));
		_bmpBar.x = 4;
		_bmpBar.y = _height - 11;
		_buffer.addChild(_bmpBar);

		_text = new TextField();
		_text.defaultTextFormat = new TextFormat('VCR OSD Mono', 12, 0x5fff5f);
		_text.embedFonts = true;
		_text.selectable = false;
		_text.multiline = false;
		_text.x = 2;
		_text.y = _bmpBar.y - 15;
		_text.autoSize = LEFT;
