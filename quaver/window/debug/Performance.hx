...
class Performance extends Sprite
{
	...
	public function new(font:Font, logoData:BitmapData = null, showAppText:Bool = false, showGraph:Bool = false)
	{
		super();
		...
		if (showAppText)
		{
			...
			appText.text = Application.current.meta.get("name") + "\nVERSION " + Application.current.meta.get("version") + "\n"
		...
		if (logoData != null)
			addChild(logo);
		#if !flash #if !html5 #if !next if (showAppText)
			addChild(appText); #end #end
		#end
		if (showGraph)
			addChild(graph);
