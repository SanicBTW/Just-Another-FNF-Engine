...
class FlxVirtualPad extends FlxSpriteGroup
{
	...
	private function createButton(X:Float, Y:Float, Width:Int, Height:Int, Graphic:String, ?Color:Int = 0xFFFFFF):FlxButton
	{
		...
		button.frames = FlxTileFrames.fromFrame(Paths.getSparrowAtlas("ui/touch/virtualpad").getByName(Graphic), FlxPoint.get(Width, Height));
		...
		#end
		addCallbacks(button, actionFormatter(Graphic));
		return button;
	}

	// i hate these functions

	private function addCallbacks(button:FlxButton, action:ActionType)
	{
	...
	// even if they are lowercase just in case
	private function actionFormatter(str:String):ActionType
	{
		// only meant to give out UI actions
