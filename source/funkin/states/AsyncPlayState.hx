...
// Just a ugly copy of playstate that supports asynchronous operations in the worst way possible
// also you can only play once per session so gotta reload the page after playing once :ghost:
class AsyncPlayState extends MusicBeatState
{
	// Cameras
	...
	override public function create()
	{
		...
		var asyncMult:MultiCallback = new MultiCallback(() ->
		{
			canPlay = true;
		...
		var chartCb:() -> Void = asyncMult.add("chart");
		var bfCb:() -> Void = asyncMult.add("chara:bf");
		var oppCb:() -> Void = asyncMult.add("chara:opps");
		var gfCb:() -> Void = asyncMult.add("chara:girlfrien");

		ChartLoader.loadVFSChart(VFSManagement.songName).then((s) ->
		{
			chartCb();

			Character.loadFromVFS(true, s.player1).then((char) ->
			{
				boyfriend = char;
				bfCb();
			});

			Character.loadFromVFS(s.player2).then((char) ->
			{
				dad = char;
				oppCb();
			});

			Character.loadFromVFS(s.gfVersion).then((char) ->
			{
				gf = char;
				gfCb();
			});
	...
	override function update(elapsed:Float)
	{
		if (canPlay)
		{
			...
			var lerpVal:Float = FlxMath.bound(elapsed * 2.4 * ((stageBuild != null) ? stageBuild.camera_settings.speed : 1), 0, 1);
	...
	private function doDeathCheck()
	{
		...
		if (stageBuild.skip.healthCheck || !canPlay)
			return;
